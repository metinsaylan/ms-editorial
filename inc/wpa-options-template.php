...
foreach ( $options as $field ) {
    ...
    ?><div class="wpa_options-input wpa_text cf">
        <label for="<?php echo $field['id']; ?>"><?php echo $field['name']; ?></label> <input name="<?php echo $field['id']; ?>" id="<?php echo $field['id']; ?>" type="<?php echo $field['type']; ?>" value="<?php if ( isset($current[ $field['id'] ]) && $current[ $field['id'] ] != "") { echo esc_html(stripslashes($current[ $field['id'] ] ) ); } ?>" <?php if ( array_key_exists( 'placeholder', $field ) ) { ?> placeholder="<?php echo $field['placeholder']; ?>" <?php } ?> /> </div><?php
